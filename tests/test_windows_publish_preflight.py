...
def _benchmark_profile_handoff(
    *,
    ready: bool = True,
    decision_profile: str | None = RESIDENT_CUDA_DQ_PROFILE_NAME,
    phase2_profile: str | None = RESIDENT_CUDA_DQ_PROFILE_NAME,
    default_route_profile: str | None = RESIDENT_CUDA_DQ_PROFILE_NAME,
    decision_check_passed: bool = True,
) -> dict[str, object]:
    profiles_agree = (
        decision_profile
        == phase2_profile
    ...
    handoff_ready = ready and profiles_agree and decision_check_passed
    return {
        "schema_version": 1,
        "ready": handoff_ready,
        ...
        "decision": {
            "present": decision_profile is not None,
            "ready": (
                decision_profile == RESIDENT_CUDA_DQ_PROFILE_NAME
                and decision_check_passed
            ),
            "check_passed": decision_check_passed,
            "source": "profile",
            "path": None,
            ...
            "schema_version": 1 if decision_profile is not None else None,
        },
        "phase2_acceptance": {
            "present": phase2_profile is not None,
            "ready": phase2_profile == RESIDENT_CUDA_DQ_PROFILE_NAME,
            "source": "profile",
            "path": None,
            "profile": phase2_profile,
            "name": "fixture_resident_cuda_dq_contract"
            if phase2_profile is not None
            else None,
            "schema_version": 1 if phase2_profile is not None else None,
        },
        "default_route": {
            "present": True,
            "ready": default_route_profile == RESIDENT_CUDA_DQ_PROFILE_NAME,
            "source": "profile",
            "path": None,
            "profile": default_route_profile,
            "name": "fixture_resident_cuda_dq_contract"
            if default_route_profile is not None
            else None,
            "schema_version": 1 if default_route_profile is not None else None,
        },
    }
...
def _release_quality_publication_guard(
    *,
    ready: bool = True,
    phase2_ready: bool | None = None,
    quality_compare_present: bool = True,
) -> dict[str, object]:
    ...
    return {
        ...
        "compatible_missing": not quality_compare_present,
        "layers_ready": guard_ready,
        "raw_matrix_status": "passed" if ready else "blocked",
        "raw_matrix_failed_check_count": 0 if ready else 1,
        "phase2_matrix_status": "passed" if phase2_ready else "blocked",
        "phase2_matrix_failed_check_count": 0 if phase2_ready else 1,
    }
...
def _release_quality_guard_publication_guard(
    *,
    ...
    final_checks_ready: bool | None = None,
    phase2_final_checks_ready: bool | None = None,
    include_final_checks: bool = True,
    include_phase2_final_checks: bool = True,
) -> dict[str, object]:
    phase2_ready = ready if phase2_ready is None else phase2_ready
    final_checks_ready = ready if final_checks_ready is None else final_checks_ready
    phase2_final_checks_ready = (
        phase2_ready
        if phase2_final_checks_ready is None
        else phase2_final_checks_ready
    )
    final_checks_compatible_missing = (
        not include_final_checks and not include_phase2_final_checks
    )
    final_checks_match = (
        include_final_checks == include_phase2_final_checks
        and (
            final_checks_compatible_missing
            or final_checks_ready == phase2_final_checks_ready
        )
    )
    final_chain_ready = final_checks_compatible_missing or (
        include_final_checks
        and include_phase2_final_checks
        and final_checks_ready
        and phase2_final_checks_ready
        and final_checks_match
    )
    guard_ready = ready and phase2_ready and final_chain_ready
    ...
    return {
        ...
        "final_fields_present": include_final_checks or include_phase2_final_checks,
        "final_checks_compatible_missing": final_checks_compatible_missing,
        "final_checks_ready": final_chain_ready,
        "final_checks_match": final_checks_match,
        "raw_final_checks_present": include_final_checks,
        "raw_final_checks_ready": final_checks_ready if include_final_checks else None,
        "phase2_final_checks_present": include_phase2_final_checks,
        "phase2_final_checks_ready": (
            phase2_final_checks_ready if include_phase2_final_checks else None
        ),
        ...
        "raw_release_matrix_check": final_checks_ready if include_final_checks else None,
        "raw_release_matrix_default_check": (
            final_checks_ready if include_final_checks else None
        ),
...
def _release_quality_final_evidence(
    *,
    ...
    phase2_ready = ready if phase2_ready is None else phase2_ready
    compatible_missing = (
        compatible_missing or (not include_raw and not include_phase2)
    )
...
def _mark_release_quality_final_evidence(
    ...
    **kwargs: object,
    ...
    for location in locations:
        ...
        elif location == "matrix_default":
            guard = payload["default_promotion_manifest"][
                "release_decision_release_quality_publication_guard"
            ]
        elif location == "default":
            guard = payload["release_decision_release_quality_publication_guard"]
        else:
            raise AssertionError(f"unknown final-evidence location: {location}")
...
def _matrix(
    ...
    *,
    ...
    release_decision_release_quality_publication_final_checks_ready: bool | None = None,
    phase2_release_decision_release_quality_publication_final_checks_ready: (
        bool | None
    ) = None,
    include_release_decision_release_quality_publication_final_checks: bool = True,
    include_phase2_release_decision_release_quality_publication_final_checks: (
        bool
    ) = True,
    ...
    default_release_decision_release_quality_publication_final_checks_ready: (
        bool | None
    ) = None,
    default_phase2_release_decision_release_quality_publication_final_checks_ready: (
        bool | None
    ) = None,
    include_default_release_decision_release_quality_publication_final_checks: (
        bool
    ) = True,
    include_default_phase2_release_decision_release_quality_publication_final_checks: (
        bool
    ) = True,
    release_benchmark_profile: str | None = RESIDENT_CUDA_DQ_PROFILE_NAME,
    release_benchmark_profile_ready: bool = True,
    release_benchmark_profile_check_passed: bool = True,
    benchmark_profile_handoff_ready: bool = True,
    benchmark_profile_decision_profile: str | None = RESIDENT_CUDA_DQ_PROFILE_NAME,
    benchmark_profile_phase2_profile: str | None = RESIDENT_CUDA_DQ_PROFILE_NAME,
    benchmark_profile_default_route_profile: str | None = RESIDENT_CUDA_DQ_PROFILE_NAME,
    benchmark_profile_decision_check_passed: bool = True,
) -> None:
    ...
    release_benchmark_ready = (
        release_benchmark_profile_ready
        and release_benchmark_profile_check_passed
        and release_benchmark_profile == RESIDENT_CUDA_DQ_PROFILE_NAME
    )
    benchmark_profile_handoff = _benchmark_profile_handoff(
        ready=benchmark_profile_handoff_ready,
        decision_profile=benchmark_profile_decision_profile,
        phase2_profile=benchmark_profile_phase2_profile,
        default_route_profile=benchmark_profile_default_route_profile,
        decision_check_passed=benchmark_profile_decision_check_passed,
    )
    benchmark_handoff_ready = bool(benchmark_profile_handoff["ready"])
    ...
    release_quality_guard = _release_quality_publication_guard(
        ready=release_quality_publication_guard_ready,
    ...
    default_release_quality_guard = _release_quality_publication_guard(
        ready=default_release_quality_publication_guard_ready,
    ...
    release_quality_policy_guard = _release_quality_guard_publication_guard(
        ...
        final_checks_ready=(
            release_decision_release_quality_publication_final_checks_ready
        ),
        phase2_final_checks_ready=(
            phase2_release_decision_release_quality_publication_final_checks_ready
        ),
        include_final_checks=(
            include_release_decision_release_quality_publication_final_checks
        ),
        include_phase2_final_checks=(
            include_phase2_release_decision_release_quality_publication_final_checks
        ),
    )
    ...
    matrix_ready = (
        ...
        and release_benchmark_ready
        and benchmark_handoff_ready
        and (
    ...
    payload["release_decision_benchmark_contract_profile"] = {
        "schema_version": 1,
        "present": release_benchmark_profile is not None,
        "ready": release_benchmark_ready,
        "check_passed": release_benchmark_profile_check_passed,
        "required_profile": RESIDENT_CUDA_DQ_PROFILE_NAME,
        "source": "profile",
        "path": None,
        "profile": release_benchmark_profile,
        "name": "fixture_resident_cuda_dq_contract"
        if release_benchmark_profile is not None
        else None,
...
def test_windows_publish_preflight_passes_consistent_bundle(tmp_path: Path):
    ...
    assert (
        payload["summary"]["matrix_release_benchmark_profile"]
        == RESIDENT_CUDA_DQ_PROFILE_NAME
    )
    assert payload["summary"]["matrix_release_benchmark_profile_ready"] is True
    assert (
        payload["summary"]["matrix_release_benchmark_profile_check_passed"]
        is True
    )
    assert payload["summary"]["matrix_benchmark_profile_handoff_ready"] is True
    assert (
        payload["summary"]["matrix_benchmark_profile_handoff_required_profile"]
        == RESIDENT_CUDA_DQ_PROFILE_NAME
    )
    assert (
        payload["summary"]["matrix_benchmark_profile_handoff_profiles_agree"]
        is True
    )
    assert (
        payload["summary"][
    ...
    assert (
        payload["summary"]["matrix_release_quality_publication_guard_present"]
        is True
    )
    assert (
        payload["summary"]["matrix_release_quality_publication_guard_ready"]
        is True
    )
    assert (
        payload["summary"][
    ...
    assert (
        payload["summary"]["matrix_release_quality_publication_guard_raw_status"]
        == "passed"
    )
    assert (
        payload["summary"][
    ...
    assert (
        checks["matrix_release_decision_benchmark_contract_profile_passed"]
        is True
    )
    assert checks["matrix_benchmark_contract_profile_handoff_passed"] is True
    assert (
        checks["default_promotion_benchmark_contract_profile_handoff_passed"]
        is True
    )
    assert (
        checks["matrix_release_decision_benchmark_profile_matches_handoff"]
        is True
    )
    assert (
        checks[
...
def test_windows_publish_preflight_blocks_matrix_benchmark_profile_drift(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(
        matrix,
        labels=labels,
        release_benchmark_profile="legacy_profile",
        release_benchmark_profile_check_passed=False,
    )
    ...
    assert checks["matrix_release_decision_benchmark_contract_profile_passed"][
        "passed"
    ] is False
    assert "matrix_release_decision_benchmark_contract_profile_passed" in payload[
        "failed_checks"
    ]
    assert checks["matrix_release_decision_benchmark_contract_profile_passed"][
        "evidence"
...
def test_windows_publish_preflight_blocks_matrix_benchmark_handoff_drift(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(
        matrix,
        labels=labels,
        benchmark_profile_decision_profile="legacy_profile",
        benchmark_profile_decision_check_passed=False,
    )
    _default_promotion(promotion)
    _manifest(manifest, matrix=matrix, labels=labels)
    _github_plan(plan, manifest=manifest, matrix=matrix, labels=labels)
    ...
    assert payload["passed"] is False
    assert payload["status"] == "blocked"
    assert checks["matrix_benchmark_contract_profile_handoff_passed"][
        "passed"
    ] is False
    assert checks["matrix_release_decision_benchmark_profile_matches_handoff"][
        "passed"
    ] is False
    assert checks[
        "matrix_benchmark_contract_profile_handoff_matches_default_promotion"
...
def test_windows_publish_preflight_blocks_default_benchmark_handoff_drift(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    assert payload["passed"] is False
    assert payload["status"] == "blocked"
    assert checks["default_promotion_benchmark_contract_profile_handoff_passed"][
        "passed"
    ] is False
    assert checks[
        "matrix_benchmark_contract_profile_handoff_matches_default_promotion"
    ...
    assert checks["default_promotion_benchmark_contract_profile_handoff_passed"][
        "evidence"
    ]["decision_profile"] == "legacy_profile"
...
def test_windows_publish_preflight_allows_missing_release_quality_publication_guard(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    matrix = tmp_path / "matrix.json"
    promotion = tmp_path / "promotion.json"
    _matrix(matrix, labels=labels, include_release_quality_publication_guard=False)
    _default_promotion(
        promotion,
...
def test_windows_publish_preflight_blocks_failed_matrix_release_quality_guard(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    matrix = tmp_path / "matrix.json"
    promotion = tmp_path / "promotion.json"
    _matrix(matrix, labels=labels, release_quality_publication_guard_ready=False)
    _default_promotion(promotion)
    _manifest(manifest, matrix=matrix, labels=labels)
    _github_plan(plan, manifest=manifest, matrix=matrix, labels=labels)
...
def test_windows_publish_preflight_blocks_failed_default_release_quality_guard(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(
        matrix,
        labels=labels,
        default_release_quality_publication_guard_ready=False,
    )
    _default_promotion(promotion, release_quality_publication_guard_ready=False)
    _manifest(manifest, matrix=matrix, labels=labels)
    _github_plan(plan, manifest=manifest, matrix=matrix, labels=labels)
...
def test_windows_publish_preflight_blocks_matrix_default_release_quality_guard_mismatch(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
...
def test_windows_publish_preflight_allows_compatible_missing_release_quality_final_checks(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(
        matrix,
        labels=labels,
        include_release_decision_release_quality_publication_final_checks=False,
        include_phase2_release_decision_release_quality_publication_final_checks=False,
        include_default_release_decision_release_quality_publication_final_checks=False,
        include_default_phase2_release_decision_release_quality_publication_final_checks=False,
    )
    _default_promotion(
        promotion,
        include_release_decision_release_quality_publication_final_checks=False,
    ...
    manifest_evidence = checks[
        "default_promotion_release_decision_release_quality_publication_guard_passed"
    ]["evidence"]
    assert payload["passed"] is True
    assert matrix_evidence["final_checks_compatible_missing"] is True
    assert matrix_evidence["raw_final_checks_present"] is False
    assert matrix_evidence["phase2_final_checks_present"] is False
    assert manifest_evidence["final_checks_compatible_missing"] is True
    assert checks[
        "matrix_release_decision_release_quality_publication_guard_matches_default_promotion"
    ]["passed"] is True
    assert checks[
...
def test_windows_publish_preflight_carries_release_quality_final_evidence(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    matrix_default_evidence = checks[
        "matrix_default_promotion_release_decision_release_quality_publication_guard_passed"
    ]["evidence"]
    manifest_evidence = checks[
        "default_promotion_release_decision_release_quality_publication_guard_passed"
    ...
    assert payload["passed"] is True
    assert matrix_evidence["final_evidence_ready"] is True
...
def test_windows_publish_preflight_allows_compatible_missing_release_quality_final_evidence(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(matrix, labels=labels)
    _default_promotion(promotion)
    _mark_release_quality_final_evidence(
        matrix,
        "matrix",
        "matrix_default",
        include_raw=False,
...
def test_windows_publish_preflight_allows_legacy_release_quality_without_final_evidence(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
...
def test_windows_publish_preflight_blocks_failed_matrix_release_quality_final_checks(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(
        matrix,
        labels=labels,
        release_decision_release_quality_publication_final_checks_ready=False,
        phase2_release_decision_release_quality_publication_final_checks_ready=False,
    )
    _default_promotion(promotion)
    _manifest(manifest, matrix=matrix, labels=labels)
    _github_plan(plan, manifest=manifest, matrix=matrix, labels=labels)
...
def test_windows_publish_preflight_blocks_failed_matrix_release_quality_final_evidence(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(matrix, labels=labels)
    _default_promotion(promotion)
    _mark_release_quality_final_evidence(matrix, "matrix", raw_ready=False)
    _mark_release_quality_final_evidence(promotion, "default")
    _manifest(manifest, matrix=matrix, labels=labels)
    _github_plan(plan, manifest=manifest, matrix=matrix, labels=labels)
...
def test_windows_publish_preflight_blocks_missing_matrix_phase2_release_quality_final_evidence(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(matrix, labels=labels)
    _default_promotion(promotion)
    _mark_release_quality_final_evidence(matrix, "matrix", include_phase2=False)
    _mark_release_quality_final_evidence(promotion, "default")
    _manifest(manifest, matrix=matrix, labels=labels)
    _github_plan(plan, manifest=manifest, matrix=matrix, labels=labels)
...
def test_windows_publish_preflight_blocks_failed_default_release_quality_final_checks(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _matrix(
        matrix,
        labels=labels,
        default_release_decision_release_quality_publication_final_checks_ready=False,
        default_phase2_release_decision_release_quality_publication_final_checks_ready=False,
    )
    _default_promotion(
        promotion,
        release_decision_release_quality_publication_final_checks_ready=False,
    ...
    assert payload["passed"] is False
    assert matrix_evidence["final_checks_ready"] is False
    assert matrix_evidence["raw_final_checks_ready"] is False
    assert manifest_evidence["final_checks_ready"] is False
    assert checks[
        "matrix_default_promotion_release_decision_release_quality_publication_guard_matches_manifest"
    ]["passed"] is True


def test_windows_publish_preflight_blocks_failed_default_release_quality_final_evidence(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
    ...
    _mark_release_quality_final_evidence(promotion, "default", raw_ready=False)
    _manifest(manifest, matrix=matrix, labels=labels)
    _github_plan(plan, manifest=manifest, matrix=matrix, labels=labels)
    ...
    assert payload["passed"] is False
    assert matrix_evidence["final_evidence_ready"] is False
    assert matrix_evidence["raw_final_evidence_ready"] is False
    assert manifest_evidence["final_evidence_ready"] is False
    assert checks[
        "matrix_default_promotion_release_decision_release_quality_publication_guard_matches_manifest"
    ]["passed"] is True


def test_windows_publish_preflight_blocks_missing_manifest_release_quality_final_checks(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
...
def test_windows_publish_preflight_blocks_missing_manifest_release_quality_final_evidence(
    tmp_path: Path,
):
    labels = ["cuda13", "cpu"]
    manifest = tmp_path / "manifest.json"
...
def test_windows_publish_preflight_cli_writes_outputs(tmp_path: Path):
    ...
    assert (
        "Benchmark profile handoff: matrix-release "
        "`resident_cuda_dq_v1`/`True`/`True`, matrix-default "
        "`True`/`resident_cuda_dq_v1`/`True`, default-promotion "
        "`True`/`resident_cuda_dq_v1`/`True`"
    ) in markdown_text
    assert (
        "Benchmark profile chain: matrix "
        "`resident_cuda_dq_v1`/`resident_cuda_dq_v1`/`resident_cuda_dq_v1`, "
        "default-promotion "
        "`resident_cuda_dq_v1`/`resident_cuda_dq_v1`/`resident_cuda_dq_v1`"
    ) in markdown_text
    ...
    assert (
        "Release quality publication guard: matrix `True`/`True`/`True`/`True`/`passed`/`passed`, "
        "matrix-default `True`/`passed`/`passed`, "
        "default-promotion `True`/`True`/`True`/`True`/`passed`/`passed`"
    ) in markdown_text
    assert (
    ...
    assert "matrix_benchmark_contract_profile_handoff_passed" in markdown_text
    assert (
