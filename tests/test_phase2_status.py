...
def _write_publish_preflight(
    ...
    *,
    ...
    benchmark_profile: str | None = RESIDENT_CUDA_DQ_PROFILE_NAME,
    ...
    benchmark_profile_passed = (
        benchmark_profile == RESIDENT_CUDA_DQ_PROFILE_NAME
        and benchmark_profile_ready
    )
    artifact_ready = ready and (
        ...
        benchmark_profile_passed or not include_benchmark_profile_handoff
    )
    summary = {
    ...
    if include_release_quality_publication_guard:
        ...
        if include_release_quality_publication_guard_final_evidence:
            ...
            final_evidence_detail_fields = {
                ...
                "raw_final_evidence_ready": final_raw_ready,
                "phase2_final_evidence_ready": final_phase2_ready,
            }
            for prefix in (
                ...
                for suffix, value in legacy_final_evidence_fields.items():
                    summary[f"{prefix}_{suffix}"] = value
                for suffix, value in final_evidence_detail_fields.items():
                    summary[f"{prefix}_{suffix}"] = value
    ...
    if include_benchmark_profile_handoff:
        summary.update(
            {
                ...
                "matrix_release_benchmark_profile_ready": benchmark_profile_passed,
                "matrix_release_benchmark_profile_check_passed": (
                    benchmark_profile_passed
                ),
                "matrix_benchmark_profile_handoff_ready": benchmark_profile_passed,
            ...
            [
                {
                    ...
                    "passed": benchmark_profile_passed,
                },
                {
        ...
        if not benchmark_profile_passed:
            failed_checks = [
                *failed_checks,
...
def _status_payload(
    *,
    ...
    publish_preflight_benchmark_profile_present: bool = True,
    publish_preflight_benchmark_profile: str | None = (
        RESIDENT_CUDA_DQ_PROFILE_NAME
    ),
    publish_preflight_benchmark_profile_ready: bool = True,
    ...
    publish_preflight_benchmark_profile_passed = (
        publish_preflight_benchmark_profile_present
        and publish_preflight_benchmark_profile_ready
        and publish_preflight_benchmark_profile == RESIDENT_CUDA_DQ_PROFILE_NAME
    )
    if publish_preflight_benchmark_profile_present:
        benchmark_profile = publish_preflight_benchmark_profile
        benchmark_profile_ready = publish_preflight_benchmark_profile_passed
        benchmark_profile_required = RESIDENT_CUDA_DQ_PROFILE_NAME
        benchmark_profiles_agree = publish_preflight_benchmark_profile_passed
    else:
        benchmark_profile = None
        benchmark_profile_ready = None
        benchmark_profile_required = None
        benchmark_profiles_agree = None
    return {
        "schema_version": 1,
        ...
        "publish_preflight": {
            ...
            **{
                f"{prefix}_{suffix}": (
                    (
                        True
                        if suffix in {"final_evidence_ready", "final_evidence_match"}
                        else None
                    )
                    if publish_preflight_release_quality_publication_guard_final_evidence_compatible_missing
                    else publish_preflight_release_quality_publication_guard_final_evidence_passed
                    if suffix != "final_evidence_match"
                    else True
                )
                if publish_preflight_release_quality_publication_guard_final_evidence_present
                else None
...
def test_phase2_status_surfaces_publish_preflight_benchmark_profile_handoff(
    tmp_path: Path,
):
    checkpoints = tmp_path / "checkpoints"
    checkpoints.mkdir()
    _write_checkpoint(checkpoints, gate=410)
    publish_preflight = tmp_path / "publish_preflight.json"
    _write_publish_preflight(publish_preflight)
    ...
    assert benchmark_check["evidence"]["present"] is True
    assert benchmark_check["evidence"]["matrix_release_benchmark_profile"] == (
        RESIDENT_CUDA_DQ_PROFILE_NAME
    )
    assert benchmark_check["evidence"]["checks_passed"] is True
    assert benchmark_check["evidence"]["statuses_passed"] is True
    assert "Benchmark profile handoff" in text
    assert "Benchmark profile checks" in text


def test_phase2_status_allows_missing_publish_preflight_benchmark_profile_handoff(
    tmp_path: Path,
):
    checkpoints = tmp_path / "checkpoints"
    checkpoints.mkdir()
    ...
    assert status["status"] == "green"
    assert benchmark_check["passed"] is True
    assert benchmark_check["evidence"]["present"] is False
    assert benchmark_check["evidence"]["matrix_release_benchmark_profile"] is None


def test_phase2_status_blocks_failed_publish_preflight_benchmark_profile_handoff(
    tmp_path: Path,
):
    checkpoints = tmp_path / "checkpoints"
    checkpoints.mkdir()
    ...
    _write_publish_preflight(
        publish_preflight,
        benchmark_profile="legacy_profile",
    ...
    assert benchmark_check["evidence"]["checks_passed"] is False
    assert benchmark_check["evidence"]["statuses_passed"] is False
    assert (
        "matrix_release_decision_benchmark_contract_profile_passed"
...
def test_phase2_status_surfaces_publish_preflight_release_quality_publication_guard(
    ...
    assert guard_check["evidence"]["final_evidence_detail_present"] is True
    assert guard_check["evidence"]["matrix_final_evidence_ready"] is True
    assert guard_check["evidence"]["matrix_raw_final_evidence_ready"] is True
    assert guard_check["evidence"]["default_promotion_phase2_final_evidence_ready"] is True
    assert (
    ...
    assert "Release quality publication guard final evidence detail" in text
...
def test_phase2_status_allows_compatible_missing_publish_preflight_release_quality_final_evidence(
    ...
    assert guard_check["evidence"]["final_evidence_detail_present"] is True
    assert guard_check["evidence"]["matrix_final_evidence_ready"] is True
    assert guard_check["evidence"]["matrix_raw_final_evidence_ready"] is None
    assert guard_check["evidence"]["matrix_phase2_final_evidence_ready"] is None
...
def test_phase2_status_blocks_failed_publish_preflight_release_quality_final_evidence(
    ...
    assert guard_check["evidence"]["final_evidence_detail_present"] is True
    assert guard_check["evidence"]["matrix_final_evidence_ready"] is False
    assert guard_check["evidence"]["matrix_raw_final_evidence_ready"] is False


def test_phase2_status_allows_legacy_publish_preflight_release_quality_final_evidence_without_detail(
    tmp_path: Path,
):
    checkpoints = tmp_path / "checkpoints"
    checkpoints.mkdir()
    _write_checkpoint(checkpoints, gate=388)
    publish_preflight = tmp_path / "publish_preflight.json"
    _write_publish_preflight(publish_preflight)
    payload = read_json(publish_preflight)
    for prefix in (
        "matrix_release_decision_release_quality_publication_guard",
        "matrix_default_promotion_release_decision_release_quality_publication_guard",
        "default_promotion_release_decision_release_quality_publication_guard",
        ...
        for suffix in (
            ...
            payload["summary"][f"{prefix}_{suffix}"] = None
    write_json(publish_preflight, payload)
    ...
    assert guard_check["passed"] is True
    assert guard_check["evidence"]["final_evidence_present"] is True
    assert guard_check["evidence"]["final_evidence_detail_present"] is False
    assert guard_check["evidence"]["final_evidence_passed"] is True
    assert guard_check["evidence"]["matrix_final_checks_ready"] is True
    assert guard_check["evidence"]["matrix_final_evidence_ready"] is None


def test_phase2_status_blocks_failed_publish_preflight_release_quality_final_evidence_detail(
    tmp_path: Path,
):
    checkpoints = tmp_path / "checkpoints"
    checkpoints.mkdir()
    ...
    assert guard_check["evidence"]["final_evidence_detail_present"] is True
    assert guard_check["evidence"]["final_evidence_passed"] is False
    assert guard_check["evidence"]["matrix_phase2_final_checks_ready"] is True
    assert guard_check["evidence"]["matrix_phase2_final_evidence_ready"] is False
...
def test_phase2_status_compare_passes_non_regression(tmp_path: Path):
    ...
    assert checks["windows_publish_preflight_benchmark_profile_handoff_preserved"] is True
    assert (
        checks[
...
def test_phase2_status_compare_flags_publish_preflight_benchmark_profile_regression(
    tmp_path: Path,
):
    baseline = tmp_path / "baseline.json"
    candidate = tmp_path / "candidate.json"
    write_json(baseline, _status_payload(gate=409))
    write_json(
        candidate,
        _status_payload(
            gate=410,
            status="attention_required",
            publish_preflight_benchmark_profile="legacy_profile",
            publish_preflight_benchmark_profile_ready=False,
        ),
    )
    ...
    assert status_check["passed"] is False
    assert (
        status_check["evidence"]["candidate"]["matrix_release_benchmark_profile"]
        == "legacy_profile"
    )
    assert (
        payload["candidate"]["publish_preflight_benchmark_profile_handoff"][
            "matrix_release_benchmark_profile"
        ]
        == "legacy_profile"
    )
...
def test_phase2_status_compare_flags_missing_publish_preflight_release_quality_final_evidence_detail(
    tmp_path: Path,
):
    baseline = tmp_path / "baseline.json"
    candidate = tmp_path / "candidate.json"
    write_json(baseline, _status_payload(gate=388))
    candidate_payload = _status_payload(gate=388)
    for prefix in (
        "matrix_release_decision_release_quality_publication_guard",
        "matrix_default_promotion_release_decision_release_quality_publication_guard",
        "default_promotion_release_decision_release_quality_publication_guard",
        ...
        for suffix in (
            ...
            candidate_payload["publish_preflight"][f"{prefix}_{suffix}"] = None
    write_json(candidate, candidate_payload)
    ...
    assert payload["status"] == "regressed"
    assert legacy_check["passed"] is True
    assert detail_check["passed"] is False
    assert (
        detail_check["evidence"]["baseline"]["final_evidence_detail_present"] is True
    )
    assert (
        detail_check["evidence"]["candidate"]["final_evidence_detail_present"]
        is False
    )
    assert (
        payload["candidate"]["publish_preflight_release_quality_publication_guard"][
