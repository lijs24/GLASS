...
__global__ void glass_apply_star_protected_isolated_cosmetic_threshold_mask_frames_f32_kernel(
    float* stack,
    std::size_t width,
    std::size_t height,
    const unsigned long long* frame_indices,
  ...
  const std::size_t frame_index = static_cast<std::size_t>(frame_indices[frame_pos]);
  float* frame = stack + frame_index * n;
  unsigned long long* frame_counts = counts + frame_pos * 10ULL;
  bool hot;
  bool cold;
  bool nonfinite;
  bool hot_candidate;
  ...
  const unsigned long long catalog_count_u64 = star_counts[frame_pos];
  const int catalog_count = catalog_count_u64 > 2147483647ULL
      ? 2147483647
      : static_cast<int>(catalog_count_u64);
  const unsigned long long offset = star_offsets[frame_pos];
  const float* frame_star_xs = catalog_count > 0 ? star_xs + offset : nullptr;
  const float* frame_star_ys = catalog_count > 0 ? star_ys + offset : nullptr;
  const std::size_t y = i / width;
  const std::size_t x = i - y * width;
  const float radius = star_protection_radii[frame_pos];
  const float radius_sq = radius * radius;
  const bool star_protected = (hot || cold) && glass_pixel_in_star_protection_f32(
      x, y, frame_star_xs, frame_star_ys, catalog_count, radius_sq);
  if (star_protected) {
    if (hot) {
      atomicAdd(&frame_counts[7], 1ULL);
    }
    if (cold) {
      atomicAdd(&frame_counts[8], 1ULL);
    }
    atomicAdd(&frame_counts[9], 1ULL);
    return;
  }
  if (hot) {
    atomicAdd(&frame_counts[0], 1ULL);
...
__global__ void glass_count_star_protected_isolated_cosmetic_threshold_mask_frames_f32_kernel(
    const float* stack,
    std::size_t width,
    std::size_t height,
    const unsigned long long* frame_indices,
...
void glass_apply_star_protected_isolated_cosmetic_threshold_mask_frames_f32_launch(
  ...
  const int blocks = static_cast<int>((n + threads - 1) / threads);
  const dim3 grid(blocks, static_cast<unsigned int>(frame_count));
  glass_apply_star_protected_isolated_cosmetic_threshold_mask_frames_f32_kernel<<<grid, threads>>>(
      stack,
      width,
      height,
      frame_indices,
...
void glass_count_star_protected_isolated_cosmetic_threshold_mask_frames_f32_launch(
  ...
  const int blocks = static_cast<int>((n + threads - 1) / threads);
  const dim3 grid(blocks, static_cast<unsigned int>(frame_count));
  glass_count_star_protected_isolated_cosmetic_threshold_mask_frames_f32_kernel<<<grid, threads>>>(
      stack,
      width,
      height,
      frame_indices,
...
__device__ void glass_swap_u16(unsigned short& a, unsigned short& b) {
  const unsigned short tmp = a;
  a = b;
  b = tmp;
}

__device__ float glass_select_kth_indexed_range_f32(
    const float* values,
    ...
    int k) {
  while (left < right) {
    const float pivot = values[indices[(left + right) >> 1]];
    int lower = left;
    int scan = left;
    int upper = right;
    while (scan <= upper) {
      const float value = values[indices[scan]];
      if (value < pivot) {
        glass_swap_u16(indices[lower], indices[scan]);
        ++lower;
        ++scan;
      } else if (value > pivot) {
        glass_swap_u16(indices[scan], indices[upper]);
        --upper;
      } else {
        ++scan;
      }
    ...
    if (k < lower) {
      ...
      return values[indices[k]];
    }
  }
  return values[indices[left]];
}
...
__device__ void glass_select_quartiles_indexed_f32(
    const float* values,
    unsigned short* indices,
    int count,
    float* median,
  ...
  for (int i = 0; i < rank_count; ++i) {
    const int rank = ranks[i];
    selected[i] = glass_select_kth_indexed_range_f32(values, indices, search_left, count - 1, rank);
    search_left = rank + 1;
  }
...
template <int MaxFrames, bool Enabled>
struct GlassIndexedQuartileSelector {
  __device__ void set(int) {}
  __device__ void select(float* values, int count, float* median, float* q25, float* q75) {
    glass_select_quartiles_f32(values, count, median, q25, q75);
  }
};

template <int MaxFrames>
struct GlassIndexedQuartileSelector<MaxFrames, true> {
  unsigned short indices[MaxFrames];
  __device__ void set(int index) { indices[index] = static_cast<unsigned short>(index); }
  __device__ void select(float* values, int count, float* median, float* q25, float* q75) {
    glass_select_quartiles_indexed_f32(values, indices, count, median, q25, q75);
  }
};
...
template <
    typename CountT,
    int MaxFrames,
    bool UnitPositiveWeights,
    bool UnitPositiveWeightMask,
    bool ReuseLocalUnitSamples,
    bool ReuseSelectedUnitSamples,
    bool ReuseIndexedUnitSamples>
__global__ void glass_integrate_resident_hardened_winsorized_sigma_f32_kernel(
  ...
  GlassIndexedQuartileSelector<MaxFrames, ReuseIndexedUnitSamples> indexed_quartiles;
  int count = 0;
  ...
  float q25 = 0.0f;
  float q75 = 0.0f;
  indexed_quartiles.select(values, count, &center0, &q25, &q75);
  ...
  if (!(first_scale > 0.0f) || !isfinite(first_scale)) {
    ...
    if (ReuseLocalUnitSamples || ReuseSelectedUnitSamples || ReuseIndexedUnitSamples) {
      for (int i = 0; i < count; ++i) {
        fallback_mean += ReuseLocalUnitSamples ? ordered_values.get(i) : values[i];
      }
    ...
    if (ReuseLocalUnitSamples || ReuseSelectedUnitSamples || ReuseIndexedUnitSamples) {
      for (int i = 0; i < count; ++i) {
        const float sample = ReuseLocalUnitSamples ? ordered_values.get(i) : values[i];
        const float delta = sample - fallback_mean;
  ...
  if (ReuseLocalUnitSamples || ReuseSelectedUnitSamples || ReuseIndexedUnitSamples) {
    for (int i = 0; i < count; ++i) {
      float value = ReuseLocalUnitSamples ? ordered_values.get(i) : values[i];
      if (value < first_low) {
        value = first_low;
  ...
  } else if (ReuseLocalUnitSamples || ReuseSelectedUnitSamples || ReuseIndexedUnitSamples) {
    for (int i = 0; i < count; ++i) {
      const float value = ReuseLocalUnitSamples ? ordered_values.get(i) : values[i];
      if (value < low_threshold) {
        low_reject += 1.0f;
  ...
  if (capacity_frame_count <= static_cast<std::size_t>(kGlassHardenedWinsorizedSmallMaxFrames)) {
    if (unit_positive_local_reuse) {
      ...
      glass_integrate_resident_hardened_winsorized_sigma_f32_kernel<
          ...
          min_samples,
          max_reject_fraction);
    } else if (unit_positive_selected_reuse) {
      glass_integrate_resident_hardened_winsorized_sigma_f32_kernel<
          CountT,
          kGlassHardenedWinsorizedSmallMaxFrames,
          false,
          ...
          min_samples,
          max_reject_fraction);
    } else if (unit_positive_index_select_reuse) {
      glass_integrate_resident_hardened_winsorized_sigma_f32_kernel<
          CountT,
          kGlassHardenedWinsorizedSmallMaxFrames,
          false,
...
void glass_integrate_resident_hardened_winsorized_sigma_f32_launch(
  ...
  glass_integrate_resident_hardened_winsorized_sigma_f32_launch_typed<float>(
      ...
      unit_positive_selected_reuse,
      unit_positive_index_select_reuse);
}
