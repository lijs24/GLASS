...
__global__ void glass_pair_grid_sum_stats_batch_f32_kernel(
    const float* stack,
  ...
  const int tile_index = static_cast<int>(blockIdx.x);
  const int source_position = static_cast<int>(blockIdx.y);
  if (tile_index >= grid_cols * grid_rows || source_position >= source_count) {
    return;
  }
  const int source_index = source_indices[source_position];
  const float* source = stack + static_cast<std::size_t>(source_index) * pixels_per_frame;
  const float* reference = stack + static_cast<std::size_t>(reference_index) * pixels_per_frame;
  ...
  if (lane == 0) {
    const std::size_t output_index =
        static_cast<std::size_t>(source_position) *
            static_cast<std::size_t>(grid_cols) * static_cast<std::size_t>(grid_rows) +
        static_cast<std::size_t>(tile_index);
    source_sum[output_index] = source_sums[0];
    source_sum2[output_index] = source_sums2[0];
    reference_sum[output_index] = reference_sums[0];
    reference_sum2[output_index] = reference_sums2[0];
    count[output_index] = counts[0];
  }
}
...
void glass_pair_grid_sum_stats_batch_f32_launch(
  ...
  constexpr int threads = 256;
  dim3 blocks(
      static_cast<unsigned int>(grid_cols * grid_rows),
      static_cast<unsigned int>(source_count));
  const std::size_t shared_bytes =
      4 * threads * sizeof(double) + threads * sizeof(unsigned long long);
  glass_pair_grid_sum_stats_batch_f32_kernel<<<blocks, threads, shared_bytes>>>(
      stack,
      source_indices,
      source_count,
      reference_index,
