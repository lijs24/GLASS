...
__global__ void glass_calibrate_fits_u16be_bzero_f32_kernel(
  ...
  const std::size_t byte_i = i * 2;
  const unsigned int bits =
      (static_cast<unsigned int>(raw_be[byte_i]) << 8) |
      static_cast<unsigned int>(raw_be[byte_i + 1]);
  float value = static_cast<float>(bits ^ 0x8000u);
...
void glass_calibrate_fits_u16be_bzero_f32_launch_stream(
  ...
  constexpr int threads = 256;
  const int blocks = static_cast<int>((n + threads - 1) / threads);
  glass_calibrate_fits_u16be_bzero_f32_kernel<<<blocks, threads, 0, stream>>>(
      raw_be,
      bias,
      dark,
      flat,
