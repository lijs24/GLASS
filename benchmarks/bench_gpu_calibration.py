...
def main() -> int:
    ...
    _, elapsed, peak_ram = run_timed(
        lambda: run_calibration_stages(plan_path, run, backend="cuda", tile_size=args.tile_size)
    )
    artifacts = read_json(run / "calibration_artifacts.json")
    backend = artifacts.get("calibrated_lights", [{}])[0].get("backend", "cuda")
    write_result(
        args.out,
        name="gpu_calibration",
