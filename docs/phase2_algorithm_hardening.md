...
### S2-Gate 374: Windows Release Matrix Release Quality Publication Guard

- Carry release-decision and default-promotion release quality publication guard
  evidence into `glass windows-release-matrix`.
- Add `release_decision_release_quality_publication_guard_passed` and
  `default_promotion_release_decision_release_quality_publication_guard_passed`
  so Windows release readiness blocks when release quality guard evidence fails,
  is lost during default promotion, or diverges between raw and Phase2 layers.
- Keep older release-decision artifacts without this optional guard non-blocking
  when no release quality guard payload or decision check is present, while
  requiring default-promotion manifests to explicitly carry that compatibility
  state.
- Surface release quality guard ready, compatible-missing, raw matrix/default,
  Phase2 matrix/default, decision-check, raw-status, Phase2-status, and
  cross-artifact match evidence in release matrix JSON and Markdown.
- Add focused ready, compatibility, release-decision failure,
  default-promotion missing, default-promotion failure, and Phase2-mismatch
  tests.
- Keep this gate Windows release-matrix scoped: no quality metric math change,
  no default quality threshold, no star detector algorithm change, no
  registration transform math change, no integration math change, no CUDA kernel
...
### S2-Gate 375: Windows Publish Preflight Release Quality Publication Guard

- Carry Windows release-matrix and default-promotion release quality publication
  guard evidence into `glass windows-publish-preflight`.
- Add final local publication checks for
  ...
  `default_promotion_release_decision_release_quality_publication_guard_passed`,
  and the matching checks between matrix/default-promotion artifacts.
- Keep older release-matrix/default-promotion artifacts without this optional
  guard non-blocking when both sides omit it, while blocking one-sided loss or
  mismatch when either side carries explicit release-quality guard evidence.
- Surface release quality guard ready, compatible-missing, raw matrix/default,
  Phase2 matrix/default, decision-check, raw-status, Phase2-status, and
  cross-artifact match evidence in publish-preflight JSON and Markdown.
- Add focused ready, compatibility, matrix failure, default-promotion failure,
  and matrix/default mismatch tests.
...
### S2-Gate 376: Phase2 Publish-Preflight Release Quality Publication Guard

- Carry the Gate375 final release quality publication guard checks into
  `glass phase2-status` and `glass phase2-status-compare`.
- Keep older publish-preflight artifacts without the final check names
  compatible when the guard chain is otherwise passing, but require all final
  checks to pass as soon as any Gate375 final check evidence is present.
- Surface the final matrix, matrix-default, default-promotion, and cross-artifact
  release quality checks in Phase2 status JSON and Markdown.
- Add status-compare regression protection so a candidate cannot drop final
  release quality check evidence that was present and passing in the baseline.
- Add focused tests for green handoff, failed final checks, and missing final
  checks in candidate status comparison.
- Keep this gate Phase2 status scoped: no quality metric math change, no default
  quality threshold, no star detector algorithm change, no registration
  transform math change, no integration math change, no CUDA kernel change, no
  runtime default change, no package upload, no GitHub release creation, and no
  real-data benchmark rerun.
...
- Carry Gate383 StackEngine publication-audit final release quality evidence
  into `glass release-promotion-decision`.
- Preserve legacy compatibility when publication-audit artifacts omit the new
  final evidence fields on both raw and Phase2 sides, while requiring
  ready/match/raw/Phase2 final evidence to pass once present.
- Block release-candidate/default-change readiness when raw publication-audit
  final evidence fails, Phase2 final evidence fails, or Phase2 drops final
  evidence that raw publication-audit carried.
- Surface final evidence readiness, match status, raw fields, and Phase2 fields
  in release-promotion JSON and Markdown.
- Add focused tests for green evidence surfacing, compatible-missing final
...
### S2-Gate 389: StackEngine Publication-Audit Final Evidence Detail Handoff

- Carry Gate388 Phase2 publish-preflight final release quality evidence detail
  into `glass stack-engine-publication-audit`.
- Preserve legacy compatibility for raw publish-preflight and Phase2 status
  artifacts that only provide historical `final_checks_*` evidence names, while
  requiring `final_evidence_*` ready/match/raw/Phase2 detail fields to pass once
  present.
- Block publication-audit readiness when raw publish-preflight detail evidence
  fails, Phase2 detail evidence fails, or Phase2 loses detail fields carried by
  raw publish-preflight.
- Include final-evidence-detail fields in raw/Phase2 publication-audit layers
  and in the raw-vs-Phase2 match contract.
- Add focused tests for green detail surfacing, compatible-missing detail,
...
### S2-Gate 391: Default-Promotion Final Evidence Detail Handoff

- Carry Gate390 release-promotion final release quality evidence detail into
  `glass default-promotion-manifest`.
- Preserve legacy compatibility for release-decision artifacts that only
  provide historical `final_checks_*` evidence names, while requiring
  `final_evidence_*` ready/match/raw/Phase2 detail fields to pass once present.
- Block default-promotion readiness when release-decision detail evidence
  fails or Phase2-side detail fields are dropped while raw release-decision
  detail is present.
- Surface final-evidence-detail readiness and raw/Phase2 matrix detail fields
  in default-promotion JSON evidence and Markdown.
- Add focused tests for green detail surfacing, compatible-missing detail,
  legacy-only compatibility, failed raw detail evidence, failed Phase2 detail
  evidence, and Phase2 detail-loss mismatch.
- Keep this gate default-promotion scoped: no quality metric math change, no
...
### S2-Gate 395: Windows Release Matrix Shared Evidence Contract

- Move the Windows release-matrix release-quality final-evidence field lists
  onto the shared report helper introduced in Gate394.
- Replace release-matrix local final-evidence detail prefix-readiness logic with
  the shared evaluator while preserving the existing release-matrix present/ready
  semantics and emitted field names.
- Validate with focused release-matrix and shared-helper tests.
- Keep this gate release-matrix report-contract scoped: no quality metric math
  change, no default quality threshold, no star detector algorithm change, no
  registration transform math change, no integration math change, no CUDA kernel
  change, no runtime default change, no package upload, no GitHub release
  creation, and no real-data benchmark rerun.
...
- Completed in S2-Gate449:
  ...
  - full pytest passed: `1071 passed in 40.65 s`.
- Artifacts:
...
### S2-Gate 450: Two-Phase Cosmetic DQ Cache Resident Regression

- Prove the Gate449 calibration-artifact DQ route with an actual two-phase
  GLASS workflow rather than a hand-authored calibration artifact fixture.
- Required work:
  - run CPU/tile calibration with `cosmetic_correction_enabled=true` on a small
    FITS fixture and verify `calibration_artifacts.json` plus DQ sidecars are
    generated by GLASS;
  - run resident CUDA integration on the same run directory without manually
    editing light-frame sidecar fields;
  - ensure resident CUDA indexes the pre-existing calibration artifacts before
    resident artifact rewrite;
  - fix any path handling gap exposed by real calibration artifact paths;
  - prove resident master/weight outputs match CPU expected valid-sample
    semantics exactly for the cosmetic hot-pixel sample;
  - run focused CUDA resident tests plus full pytest before checkpoint/commit.
- This gate directly exercises the DQ/mask pipeline contract across CPU/tile
  calibration and resident CUDA integration.
- Completed in S2-Gate450:
  - added a two-phase resident CUDA regression that first runs CPU/tile
    calibration with cosmetic correction and then runs resident CUDA on the
    same run directory with no manual sidecar edits;
  - fixed calibration artifact DQ path resolution for GLASS-generated relative
    `dq_mask_path` values that already include the run path;
  - validated that the resident master excludes the finite cosmetic hot pixel
    and that the weight map drops from 2 to 1 only at the flagged sample;
  - synthetic two-phase validation matched CPU expected master/weight exactly:
    `master_max_abs=0`, `weight_max_abs=0`, CPU calibration `0.2379 s`,
    resident run `0.1307 s`;
