...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| StackEngine combine policies | clean-room derivation | General weighted statistics | Mean, median, sum, and weighted mean over valid samples with optional maps. | TBD | TBD | Project-defined | Implemented from basic statistical definitions. | Robust estimators added gate by gate. |
| StackEngine rejection policies | clean-room derivation / paper TBD | TBD | Sigma, MAD, median sigma, percentile, min/max, and winsorized sigma rejection. | TBD | TBD | Project-defined unless cited | Implementation must be independently written from public formulas or project derivation. | Exact defaults may change after validation. |
| Data-quality mask bitfield | clean-room derivation | Project pipeline requirements | Internal bitfield for invalid, saturated, corrected, warped, LN-excluded, and rejected pixels. | TBD | TBD | Project-defined | Bit assignments are project-owned API. | May expand with camera-specific flags. |
