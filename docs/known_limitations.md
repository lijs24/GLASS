...
- The public GPU master-frame helper now records accurate
  ...
  of S2-Gate 709. S2-Gate 710 adds finite-sample DQ masks for this helper:
  non-finite source samples are excluded from the mean, and output pixels with
  no finite inputs are marked `NO_DATA`. The helper still does not consume
  source DQ sidecars. Full source-DQ-aware master-frame DQ remains owned by the
  CPU/tile StackEngine sink path and the resident master DQ sidecar path.
