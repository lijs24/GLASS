...
def star_local_max_mask(data: Any, threshold: float) -> np.ndarray:
    ...
    return glass_cuda.star_local_max_mask_f32(data, threshold)


def star_candidates(data: Any, threshold: float, max_candidates: int = 4096) -> dict[str, Any]:
    import glass_cuda

    return glass_cuda.star_candidates_f32(data, threshold, max_candidates)


def star_top_candidates(data: Any, threshold: float, max_candidates: int = 4096) -> dict[str, Any]:
    import glass_cuda

    return glass_cuda.star_top_candidates_f32(data, threshold, max_candidates)
