...
def _release_quality_publication_guard_present(summary: dict[str, Any]) -> bool:
    return any(summary.get(field) is not None for field in _RELEASE_QUALITY_PUBLICATION_GUARD_FIELDS)
...
def _release_quality_publication_guard_final_checks_present(
    ...
    return any(
        summary.get(field) is not None
        for field in _RELEASE_QUALITY_PUBLICATION_GUARD_FINAL_CHECK_FIELDS
    )
...
def _release_quality_publication_guard_final_evidence_present(
    summary: dict[str, Any],
) -> bool:
    return any(
        summary.get(field) is not None
        for field in _RELEASE_QUALITY_PUBLICATION_GUARD_FINAL_EVIDENCE_FIELDS
    )


def _release_quality_publication_guard_final_evidence_detail_present(
    summary: dict[str, Any],
) -> bool:
    return any(
        summary.get(field) is not None
        for field in _RELEASE_QUALITY_PUBLICATION_GUARD_FINAL_EVIDENCE_DETAIL_FIELDS
    )


def _release_quality_publication_guard_legacy_final_evidence_present(
    summary: dict[str, Any],
) -> bool:
    return any(
        summary.get(field) is not None
        for field in _RELEASE_QUALITY_PUBLICATION_GUARD_LEGACY_FINAL_EVIDENCE_FIELDS
    )


def _release_quality_publication_guard_final_evidence_layer_ready(
    summary: dict[str, Any],
    *,
    prefix: str,
    ready_suffix: str,
    ...
    final_ready = summary.get(f"{prefix}_{ready_suffix}")
    final_match = summary.get(f"{prefix}_{match_suffix}")
    raw_ready = summary.get(f"{prefix}_{raw_ready_suffix}")
    phase2_ready = summary.get(f"{prefix}_{phase2_ready_suffix}")
    if (
        final_ready is None
...
def _release_quality_publication_guard_legacy_final_evidence_layer_ready(
    summary: dict[str, Any],
    *,
    prefix: str,
) -> bool:
    return _release_quality_publication_guard_final_evidence_layer_ready(
        summary,
        prefix=prefix,
...
def _release_quality_publication_guard_legacy_final_evidence_ready(
    summary: dict[str, Any],
) -> bool:
    return _release_quality_publication_guard_legacy_final_evidence_present(
        summary
    ) and all(
        _release_quality_publication_guard_legacy_final_evidence_layer_ready(
            summary,
            prefix=prefix,
        )
...
def _release_quality_publication_guard_final_evidence_detail_ready(
    summary: dict[str, Any],
) -> bool:
    if not _release_quality_publication_guard_final_evidence_detail_present(summary):
        return False
    detail_evidence = {
    ...
    return _release_quality_final_evidence_detail_ready(
        detail_evidence,
        detail_fields=_RELEASE_QUALITY_PUBLICATION_GUARD_FINAL_EVIDENCE_DETAIL_FIELDS,
        prefixes=_RELEASE_QUALITY_PUBLICATION_GUARD_FINAL_EVIDENCE_PREFIXES,
        allow_partial_layer_ready=False,
    )


def _release_quality_publication_guard_final_evidence_ready(
    summary: dict[str, Any],
) -> bool:
    if not _release_quality_publication_guard_final_evidence_present(summary):
        return True
    if _release_quality_publication_guard_final_evidence_detail_present(summary):
        return _release_quality_publication_guard_final_evidence_detail_ready(
            summary
        )
    return _release_quality_publication_guard_legacy_final_evidence_ready(
        summary
    )
...
def _release_quality_publication_guard_ready(summary: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and _release_quality_publication_guard_final_evidence_ready(summary)
    )
