...
def _release_decision_release_quality_publication_guard(
    decision: dict[str, Any],
) -> dict[str, Any]:
    guard = (
    ...
    return {
        ...
        **{
            field: guard.get(field)
            for field in final_evidence_field_names
            if field
            not in {
                "final_evidence_compatible_missing",
                "final_evidence_ready",
...
def build_default_promotion_manifest(
    *,
    ...
    release_decision_release_quality_guard = (
        _release_decision_release_quality_publication_guard(decision)
    )
    ...
    checks = [
        ...
        _check(
            ...
            release_decision_release_quality_guard.get("ready") is True,
            release_decision_release_quality_guard,
            note="Required when the release decision supplies StackEngine publication release quality guard evidence.",
        ),
        _check(
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    release_decision_release_quality_guard = (
        payload.get("release_decision_release_quality_publication_guard") or {}
    )
    resident_fastpath_release_handoff = (
    ...
    lines = [
        ...
        (
            ...
            f"ready=`{release_decision_release_quality_guard.get('ready')}` "
            f"check=`{release_decision_release_quality_guard.get('decision_check_passed')}` "
            "guard-present="
            f"`{release_decision_release_quality_guard.get('release_quality_guard_present')}` "
            "compatible-missing="
            f"`{release_decision_release_quality_guard.get('compatible_missing')}`"
        ),
        (
            "- Release quality publication sources: "
            "raw-status="
            f"`{release_decision_release_quality_guard.get('raw_matrix_raw_status')}` "
            "phase2-status="
            f"`{release_decision_release_quality_guard.get('phase2_matrix_raw_status')}` "
            "raw-phase2="
            f"`{release_decision_release_quality_guard.get('raw_matrix_phase2_status')}` "
            "phase2-phase2="
            f"`{release_decision_release_quality_guard.get('phase2_matrix_phase2_status')}`"
        ),
        (
            "- Release quality publication final checks: "
            f"ready=`{release_decision_release_quality_guard.get('final_checks_ready')}` "
            f"match=`{release_decision_release_quality_guard.get('final_checks_match')}` "
            "compatible-missing="
            f"`{release_decision_release_quality_guard.get('final_checks_compatible_missing')}` "
            "raw-present="
            f"`{release_decision_release_quality_guard.get('raw_final_checks_present')}` "
            "phase2-present="
            f"`{release_decision_release_quality_guard.get('phase2_final_checks_present')}`"
        ),
        (
            "- Release quality publication final evidence: "
            f"ready=`{release_decision_release_quality_guard.get('final_evidence_ready')}` "
            f"match=`{release_decision_release_quality_guard.get('final_evidence_match')}` "
            "compatible-missing="
            f"`{release_decision_release_quality_guard.get('final_evidence_compatible_missing')}` "
            "raw-present="
        ...
        (
            ...
            "raw-matrix="
            f"`{release_decision_release_quality_guard.get('raw_release_matrix_check')}` "
            "raw-manifest="
            f"`{release_decision_release_quality_guard.get('raw_release_matrix_manifest_match_check')}` "
            "phase2-matrix="
            f"`{release_decision_release_quality_guard.get('phase2_release_matrix_check')}` "
            "phase2-manifest="
            f"`{release_decision_release_quality_guard.get('phase2_release_matrix_manifest_match_check')}`"
        ),
        (
