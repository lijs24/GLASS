...
def _release_quality_guard_publication_fields(
    source: dict[str, Any],
    *,
    output_prefix: str,
) -> dict[str, Any]:
    guard = (
        source.get("release_decision_release_quality_publication_guard")
        if isinstance(
            source.get("release_decision_release_quality_publication_guard"),
            dict,
        )
    ...
    def field(flat_name: str, *guard_names: str) -> Any:
        flattened = source.get(
            f"release_decision_release_quality_publication_guard_{flat_name}"
        )
        if flattened is not None:
            return flattened
        for guard_name in guard_names:
    ...
    return {
        ...
        f"{output_prefix}_passed": field("passed", "passed"),
        f"{output_prefix}_checks_passed": field("checks_passed", "checks_passed"),
        f"{output_prefix}_compatible_missing": field(
            "compatible_missing",
            "compatible_missing",
        ),
        f"{output_prefix}_release_quality_present": field(
        ...
        f"{output_prefix}_final_evidence_fields_present": field(
            "final_evidence_fields_present",
            "final_evidence_fields_present",
        ),
        **{
            f"{output_prefix}_{name}": field(name, name)
            for name in _RELEASE_QUALITY_PUBLICATION_FINAL_EVIDENCE_FIELDS
        },
        f"{output_prefix}_decision_check_ready": field(
            "decision_check_ready",
            "decision_check_ready",
        ),
        f"{output_prefix}_checks_ready": field("checks_ready", "checks_ready"),
        f"{output_prefix}_layers_ready": field("layers_ready", "layers_ready"),
        f"{output_prefix}_raw_matrix_raw_status": field(
            "raw_matrix_raw_status",
            "raw_matrix_raw_status",
        ),
        f"{output_prefix}_raw_matrix_phase2_status": field(
            "raw_matrix_phase2_status",
            "raw_matrix_phase2_status",
        ),
        f"{output_prefix}_raw_matrix_check_passed": field(
            "raw_matrix_check_passed",
            "raw_matrix_check_passed",
        ),
        f"{output_prefix}_raw_default_promotion_check_passed": field(
            "raw_default_promotion_check_passed",
            "raw_default_promotion_check_passed",
        ),
        f"{output_prefix}_raw_matrix_default_match_check": field(
            "raw_matrix_default_match_check",
            "raw_matrix_default_match_check",
        ),
        f"{output_prefix}_raw_matrix_manifest_match_check": field(
        ...
        f"{output_prefix}_phase2_matrix_phase2_status": field(
            "phase2_matrix_phase2_status",
            "phase2_matrix_phase2_status",
        ),
        f"{output_prefix}_phase2_matrix_check_passed": field(
            "phase2_matrix_check_passed",
            "phase2_matrix_check_passed",
        ),
        f"{output_prefix}_phase2_default_promotion_check_passed": field(
            "phase2_default_promotion_check_passed",
            "phase2_default_promotion_check_passed",
        ),
        f"{output_prefix}_phase2_matrix_default_match_check": field(
            "phase2_matrix_default_match_check",
            "phase2_matrix_default_match_check",
        ),
        f"{output_prefix}_phase2_matrix_manifest_match_check": field(
...
def _release_quality_guard_publication_evidence(
    ...
    *,
    ...
    evidence = {
        ...
        "passed": summary.get(f"{prefix}_passed"),
        "checks_passed": summary.get(f"{prefix}_checks_passed"),
        "compatible_missing": summary.get(f"{prefix}_compatible_missing"),
        ...
        "final_evidence_fields_present": summary.get(
            f"{prefix}_final_evidence_fields_present"
        ),
        **{
            name: summary.get(f"{prefix}_{name}")
            for name in _RELEASE_QUALITY_PUBLICATION_FINAL_EVIDENCE_FIELDS
        },
        "decision_check_ready": summary.get(f"{prefix}_decision_check_ready"),
        "checks_ready": summary.get(f"{prefix}_checks_ready"),
        "layers_ready": summary.get(f"{prefix}_layers_ready"),
        "raw_matrix_raw_status": summary.get(f"{prefix}_raw_matrix_raw_status"),
        "raw_matrix_phase2_status": summary.get(
            f"{prefix}_raw_matrix_phase2_status"
        ),
        "raw_matrix_check_passed": summary.get(
            f"{prefix}_raw_matrix_check_passed"
        ),
        "raw_default_promotion_check_passed": summary.get(
            f"{prefix}_raw_default_promotion_check_passed"
        ),
        "raw_matrix_default_match_check": summary.get(
            f"{prefix}_raw_matrix_default_match_check"
        ),
        ...
        "phase2_matrix_phase2_status": summary.get(
            f"{prefix}_phase2_matrix_phase2_status"
        ),
        "phase2_matrix_check_passed": summary.get(
            f"{prefix}_phase2_matrix_check_passed"
        ),
        "phase2_default_promotion_check_passed": summary.get(
            f"{prefix}_phase2_default_promotion_check_passed"
        ),
        "phase2_matrix_default_match_check": summary.get(
            f"{prefix}_phase2_matrix_default_match_check"
        ),
    ...
    if not final_evidence_fields_present:
        evidence.update(
            {
                "final_evidence_fields_present": False,
                "final_evidence_compatible_missing": True,
                "final_evidence_ready": True,
                "final_evidence_match": True,
                "raw_final_evidence_present": False,
        ...
        evidence["final_evidence_ready"] = (
            evidence.get("final_evidence_ready") is True
        )
        evidence["final_evidence_match"] = (
            evidence.get("final_evidence_match") is True
        )
        evidence["raw_final_evidence_present"] = (
            evidence.get("raw_final_evidence_present") is True
        )
...
def _release_quality_guard_publication_optional_ready(
    summary: dict[str, Any],
    *,
    prefix: str,
) -> bool:
    evidence = _release_quality_guard_publication_evidence(
        summary,
        prefix=prefix,
    )
    if evidence.get("present") is not True:
    ...
    if not final_checks_ok:
        return False
    compatible_missing_final_evidence = (
        evidence.get("final_evidence_compatible_missing") is True
        and evidence.get("raw_final_evidence_present") is not True
        and evidence.get("phase2_final_evidence_present") is not True
    )
    if (
        compatible_missing_final_evidence
        or (
            evidence.get("final_evidence_fields_present") is not True
            and evidence.get("raw_final_evidence_present") is not True
            and evidence.get("phase2_final_evidence_present") is not True
        )
    ):
...
_RELEASE_QUALITY_GUARD_PUBLICATION_MATCH_FIELDS = (
    "present",
    "ready",
    "decision_check_passed",
    "status",
...
def _release_quality_guard_publication_matches(
    left: dict[str, Any],
    *,
    left_prefix: str,
    right: dict[str, Any],
    ...
    left_evidence = _release_quality_guard_publication_evidence(
        left,
        prefix=left_prefix,
    )
    right_evidence = _release_quality_guard_publication_evidence(
        right,
        prefix=right_prefix,
    )
    return all(
        left_evidence.get(field) == right_evidence.get(field)
        for field in _RELEASE_QUALITY_GUARD_PUBLICATION_MATCH_FIELDS
    )
...
def _matrix_summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        **_release_quality_guard_publication_fields(
            payload,
            output_prefix="release_decision_release_quality_publication_guard",
        ),
        **_release_direct_publication_guard_fields(
            promotion,
            output_prefix=(
    ...
    *,
    ...
    checks = [
        ...
        _check(
            ...
            _release_quality_guard_publication_optional_ready(
                matrix_info,
                prefix="release_decision_release_quality_publication_guard",
            ),
            _release_quality_guard_publication_evidence(
                matrix_info,
                prefix="release_decision_release_quality_publication_guard",
            ),
        ),
        ...
        _check(
            ...
            _release_quality_guard_publication_matches(
                matrix_info,
                left_prefix="release_decision_release_quality_publication_guard",
                right=promotion_info,
                right_prefix="release_decision_release_quality_publication_guard",
            ),
            {
                "windows_release_matrix": (
                    _release_quality_guard_publication_evidence(
    ...
    return {
        ...
        "summary": {
            ...
            "matrix_release_decision_release_quality_publication_guard_check_passed": (
                matrix_info.get(
                    "release_decision_release_quality_publication_guard_check_passed"
                )
            ),
            "matrix_release_decision_release_quality_publication_guard_layers_ready": (
                matrix_info.get(
            ...
            "matrix_release_decision_release_quality_publication_guard_phase2_status": (
                matrix_info.get(
            ...
            "matrix_release_decision_release_quality_publication_guard_final_evidence_match": (
                matrix_info.get(
                    "release_decision_release_quality_publication_guard_final_evidence_match"
                )
            ),
            "matrix_release_decision_release_quality_publication_guard_raw_final_evidence_ready": (
                matrix_info.get(
                    "release_decision_release_quality_publication_guard_raw_final_evidence_ready"
                )
            ),
            ...
            "matrix_default_promotion_release_decision_release_quality_publication_guard_raw_status": (
                matrix_info.get(
                    "default_promotion_release_decision_release_quality_publication_guard_raw_matrix_raw_status"
                )
            ),
            ...
            "default_promotion_release_decision_release_quality_publication_guard_check_passed": (
                promotion_info.get(
                    "release_decision_release_quality_publication_guard_check_passed"
                )
            ),
            "default_promotion_release_decision_release_quality_publication_guard_layers_ready": (
                promotion_info.get(
                    "release_decision_release_quality_publication_guard_layers_ready"
                )
            ),
            "default_promotion_release_decision_release_quality_publication_guard_raw_status": (
                promotion_info.get(
                    "release_decision_release_quality_publication_guard_raw_matrix_raw_status"
                )
            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            "- Release-quality publication guard: "
            "matrix "
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_present')}`/"
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_ready')}`/"
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_check_passed')}`/"
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_layers_ready')}`/"
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_raw_status')}`/"
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_phase2_status')}`, "
            "matrix-default "
            f"`{summary.get('matrix_default_promotion_release_decision_release_quality_publication_guard_ready')}`/"
            f"`{summary.get('matrix_default_promotion_release_decision_release_quality_publication_guard_raw_status')}`/"
            f"`{summary.get('matrix_default_promotion_release_decision_release_quality_publication_guard_phase2_status')}`, "
            "default-promotion "
            f"`{summary.get('default_promotion_release_decision_release_quality_publication_guard_present')}`/"
            f"`{summary.get('default_promotion_release_decision_release_quality_publication_guard_ready')}`/"
            f"`{summary.get('default_promotion_release_decision_release_quality_publication_guard_check_passed')}`/"
        ...
        (
            "- Release-quality publication final evidence: "
            "matrix "
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_final_evidence_ready')}`/"
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_final_evidence_match')}`/"
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_raw_final_evidence_ready')}`/"
            f"`{summary.get('matrix_release_decision_release_quality_publication_guard_phase2_final_evidence_ready')}`, "
            "matrix-default "
            f"`{summary.get('matrix_default_promotion_release_decision_release_quality_publication_guard_final_evidence_ready')}`/"
            f"`{summary.get('matrix_default_promotion_release_decision_release_quality_publication_guard_final_evidence_match')}`/"
            f"`{summary.get('matrix_default_promotion_release_decision_release_quality_publication_guard_raw_final_evidence_ready')}`/"
            f"`{summary.get('matrix_default_promotion_release_decision_release_quality_publication_guard_phase2_final_evidence_ready')}`, "
            "default-promotion "
            f"`{summary.get('default_promotion_release_decision_release_quality_publication_guard_final_evidence_ready')}`/"
