...
def _load_gpwbpp_run(run: str | Path) -> dict[str, Any]:
    root = Path(run)
...
def summarize_wbpp_speedup(
    ...
    *,
    ...
    gp = _load_gpwbpp_run(gpwbpp_run)
    ...
    summary = {
        ...
        "gpwbpp_faster": speedup is not None and speedup > 1.0,
        ...
        "clean_room": {
            "status": "compliant",
            "note": (
                "This summary uses GPWBPP artifacts and user-generated PixInsight/WBPP black-box timing/output "
...
def write_speedup_markdown(path: str | Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# GPWBPP vs WBPP Speedup Summary",
        "",
        f"- GPWBPP elapsed: {gp['elapsed_s']:.3f} s",
        ...
        f"- GPWBPP backend: {gp.get('backend')}",
        f"- GPWBPP memory mode: {gp.get('memory_mode')}",
