...
- Exposed the new resident methods through the Python `gpwbpp_cuda.ResidentCalibratedStack` wrapper.
...
## Commands run
- `cmd.exe /c '"C:\\Program Files (x86)\\Microsoft Visual Studio\\2022\\BuildTools\\Common7\\Tools\\VsDevCmd.bat" -arch=x64 -host_arch=x64 && "C:\\Users\\ljs\\WORK\\astro\\gpuwbpp\\.venv\\Scripts\\cmake.exe" --build build\\native-cuda --target _gpwbpp_cuda_native -j 8'`
...
- Output run: `C:\gpwbpp_runs\final_m38_h_200\gpwbpp_resident_similarity_catalog_pair_grid96`
...
- The work reused GPWBPP CUDA kernels and clean-room catalog/pixel metric registration logic.
