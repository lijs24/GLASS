...
- StackEngine runtime default: ready=`False` acceptance=`failed` pipeline=`passed`
- Runtime default counts: acceptance-legacy=`1` acceptance-failed-masters=`1` pipeline-failed-outputs=`0` explicit-cuda=`1`
...
- PASS: `doctor_schema_version` - {'actual': 1, 'required': 1, 'path': 'runs\\checkpoints\\s2_gate_296_fixtures\\acceptance_runtime_default_failed\\doctor.json'}
...
- FAIL: `default_promotion_acceptance_stack_engine_runtime_default_passed` - {'status': 'failed', 'check_present': True, 'check_passed': False, 'phase2_check_passed': False, 'master_count': 3, 'legacy_master_count': 1, 'failed_master_count': 1, 'failed_output_count': 0, 'explicit_cuda_fast_path_count': 1, 'failed_masters': [{'engine_family': 'legacy', 'group_id': 'bias_0', 'reason': 'legacy_master_accumulator'}], 'failed_outputs': []}
