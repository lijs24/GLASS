...
- Date: 2026-05-12
- Scope: Add structured GLASS stage timing artifacts for future PixInsight/WBPP speed comparison.
- Related gates: strengthens Gate 12 reporting/resume diagnostics and prepares Gate 13 timing comparison.

## Completed

- Added `run_timing.json` generation for `glass run`, `glass audit`, and resumed pipeline execution.
...
- `.\\.venv\\Scripts\\python.exe -m py_compile src/glass/cli.py src/glass/report/html_report.py tests/test_pipeline_fixture.py`
...
- `.\\.venv\\Scripts\\glass.exe run --plan runs/real_m5_lum_subset/processing_plan.json --out runs/real_m5_lum_subset/glass_cuda_timed_run --backend cuda --until-stage integration --local-normalization off --integration-weighting none --integration-rejection none --tile-size 1024`
- `.\\.venv\\Scripts\\glass.exe report --run runs/real_m5_lum_subset/glass_cuda_timed_run --out runs/real_m5_lum_subset/glass_cuda_timed_run/report.html`
...
- `runs/real_m5_lum_subset/glass_cuda_timed_run/run_timing.json`
- `runs/real_m5_lum_subset/glass_cuda_timed_run/report.html`
- `runs/real_m5_lum_subset/glass_cuda_timed_run/integration/master_Lum.fits`

## Known Limitations

- Timing currently measures Python stage wall time inside GLASS, not external process startup overhead.
- `glass report` can display existing timing data, but a manually invoked report command does not yet append its own report-stage timing into the already rendered HTML.
...
- Use `run_timing.json` as the GLASS timing source when finalizing the WBPP black-box comparison package.
