...
- Release decision release-quality publication guard: ready=`False` check=`True` guard-present=`True` compatible-missing=`False`
- Release quality publication final checks: ready=`True` match=`True` raw=`True` phase2=`True`
- Release quality publication final evidence: ready=`False` match=`False` raw=`False` phase2=`True`
...
- Default promotion release quality final evidence: ready=`True` match=`True` raw=`True` phase2=`True`
...
- PASS: `doctor_schema_version` - {'actual': 1, 'required': 1, 'path': 'C:\\Users\\ljs\\WORK\\astro\\gpuwbpp\\runs\\checkpoints\\s2_gate_386_fixtures\\failed_direct_raw_final_evidence\\doctor.json'}
