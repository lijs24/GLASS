...
- Added a CPU fallback in `src/glass_cuda.py` so CPU-only import paths remain
  functional.
- Exposed the primitive through `glass.gpu.registration`.
- Extended `benchmarks/compare_astroalign_gpu_alignment.py` to record GPU matrix
  metrics for both the astroalign matrix and the GLASS-owned catalog-similarity
  matrix.
...
- `C:\glass_runs\final_m38_h_200\astroalign_vs_glass_gpu_pair_S000061_S000062_full_benchmark_v15_matrix_metrics.json`
...
- CUDA matrix metrics for the astroalign matrix: 0.0416 s, RMS 76.35 ADU, NCC
  0.9767.
- GLASS-owned CUDA grid-top catalog similarity: 2.954 s.
