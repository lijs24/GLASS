...
- Added coverage-aware comparison metrics to `gpwbpp compare`.
- New CLI options:
  - `--gpwbpp-coverage-map`;
  - `--min-coverage`.
...
- Coverage-masked compare report: `C:\gpwbpp_runs\final_m38_h_200\gpwbpp_resident_triangle_193_wbpp_failed_excluded_lanczos3\compare_vs_wbpp_fastintegration_scaled_coverage190.html`.
...
- Coverage masking currently uses only the GPWBPP coverage map. It does not consume a reference/WBPP coverage map.
...
- Compliant. The comparison uses GPWBPP maps and user-generated WBPP black-box output only.
