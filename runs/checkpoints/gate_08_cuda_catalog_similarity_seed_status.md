...
Status: completed incremental pure-GPU registration seed. This is not the final GPWBPP star descriptor/RANSAC matcher; it is the first CUDA primitive that estimates a similarity transform directly from two bounded star catalogs without astroalign supplying matched pairs.
...
result = gpwbpp_cuda.estimate_similarity_from_catalogs_f32(reference_catalog[:,0], reference_catalog[:,1], moving_catalog[:,0], moving_catalog[:,1], 0.15, 3.0)
print(result)
...
Compliant. This uses generic two-point 2D similarity geometry and GPWBPP CUDA code. No official WBPP/PJSR source was read or copied.
