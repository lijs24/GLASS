...
- Release quality publication final checks: ready=`True` match=`True` raw=`None` phase2=`None`
...
- Default promotion release quality final checks: ready=`True` match=`True` raw=`None` phase2=`None`
...
- PASS: `doctor_schema_version` - {'actual': 1, 'required': 1, 'path': 'C:\\Users\\ljs\\WORK\\astro\\gpuwbpp\\runs\\checkpoints\\s2_gate_380_fixtures\\compatible_missing_final_checks\\doctor.json'}
