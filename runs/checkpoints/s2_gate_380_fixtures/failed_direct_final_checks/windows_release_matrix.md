...
- Release decision release-quality publication guard: ready=`False` check=`False` guard-present=`True` compatible-missing=`False`
- Release quality publication final checks: ready=`False` match=`True` raw=`False` phase2=`False`
...
- PASS: `doctor_schema_version` - {'actual': 1, 'required': 1, 'path': 'C:\\Users\\ljs\\WORK\\astro\\gpuwbpp\\runs\\checkpoints\\s2_gate_380_fixtures\\failed_direct_final_checks\\doctor.json'}
