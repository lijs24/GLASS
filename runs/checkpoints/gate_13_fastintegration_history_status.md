...
Completed content:
- Added `src/gpwbpp/report/wbpp_history.py` to parse user-generated XISF `PixInsight:ProcessingHistory`.
...
- Added `gpwbpp blackbox-history --master ... --out ...`.
...
- `.\.venv\Scripts\python.exe -m ruff check src\gpwbpp\report\wbpp_history.py src\gpwbpp\cli.py tests\test_wbpp_history.py`
- `.\.venv\Scripts\python.exe -m gpwbpp.cli blackbox-history --master "C:\gpwbpp_runs\final_m38_h_200\pixinsight_wbpp_blackbox\master\masterLight_BIN-1_9600x6422_EXPOSURE-600.00s_FILTER-H_mono_fastIntegration.xisf" --out "C:\gpwbpp_runs\final_m38_h_200\wbpp_fastintegration_history_cli.json"`
...
- Artifact: `C:\gpwbpp_runs\final_m38_h_200\wbpp_fastintegration_history_cli.json`.
...
- It does not feed WBPP transforms or accepted-frame masks into GPWBPP automatically.
...
Next step:
- Compare the extracted WBPP diagnostic table against GPWBPP's own star-registration table and improve GPWBPP's transform/warp implementation where differences are scientifically meaningful.
